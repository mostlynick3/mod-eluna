...
namespace LuaPlayer
{
     ...
     */
    int GetAchievementCriteriaProgress(Eluna* E, Player* player)
    {
        uint32 criteriaId = E->CHECKVAL<uint32>(2);
    ...
    int CanCompleteRepeatableQuest(Eluna* E, Player* player)
    {
        ...
        E->Push(player->CanCompleteRepeatableQuest(quest));
        return 1;
    }
    ...
    int CanRewardQuest(Eluna* E, Player* player)
    {
        ...
        E->Push(player->CanRewardQuest(quest, true)); // Modify the second argument as needed
        return 1;
    }
    ...
    int GetMailCount(Eluna* E, Player* player)
    {
        ...
        if (cache)
        {
            E->Push(static_cast<uint32>(cache->MailCount));
        }
        else
        {
    ...
    */
    ...
    {
        auto slotIndex = E->CHECKVAL<uint32>(2);

        E->Push(player->GetGlyph(slotIndex));
        return 1;
    }
    ...
    int GossipMenuAddItem(Eluna* E, Player* player)
    {
        ...
        {
            return luaL_error(E->L, "GossipMenuItem not added. Reached Max amount of possible GossipMenuItems in this GossipMenu");
        }
        return 0;
    }
     ...
     * @param uint32 value
     */
    int UpdatePlayerSetting(Eluna* E, Player* player)
    {
        std::string source = E->CHECKVAL<std::string>(2);
        uint32 index = E->CHECKVAL<uint32>(3);
     ...
     * @param uint32 index
     */
    int GetPlayerSettingValue(Eluna* E, Player* player)
    {
        std::string source = E->CHECKVAL<std::string>(2);
        uint32 index = E->CHECKVAL<uint32>(3);
     ...
     */
    int GetTrader(Eluna* E, Player* player)
    {
        E->Push(player->GetTrader());
        return 1;
    }
     ...
     */
    int SetSpellPower(Eluna* E, Player* player)
    {
        int value  = E->CHECKVAL<int>(2);
        bool apply = E->CHECKVAL<bool>(3, false);
