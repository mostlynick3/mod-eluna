...
namespace LuaGlobalFunctions
{
    ...
    */
     ...
     *     TICKET_EVENT_ON_UPDATE_LAST_CHANGE              = 2,    // (event, ticket)
    ...
    {
        ELUNA_LOG_INFO("{}", GetStackAsString(E));
        return 0;
    }
    ...
    {
        ELUNA_LOG_ERROR("{}", GetStackAsString(E));
        return 0;
    }
    ...
    {
        ELUNA_LOG_DEBUG("{}", GetStackAsString(E));
        return 0;
    }
    ...
    */
     ...
     */
    //int HttpRequest(Eluna* E)
    //{
    //    std::string httpVerb = E->CHECKVAL<std::string>(1);
    //    std::string url = E->CHECKVAL<std::string>(2);
    //    std::string body;
    ...
    //    if (!lua_istable(E->L, headersIdx) && lua_isstring(E->L, headersIdx) && lua_isstring(E->L, headersIdx + 1))
    //    {
    //        body = E->CHECKVAL<std::string>(3);
    //        bodyContentType = E->CHECKVAL<std::string>(4);
    //        headersIdx = 5;
    //        callbackIdx = 5;
    //    }

    //    if (lua_istable(E->L, headersIdx))
    //    {
    //        ++callbackIdx;

    //        lua_pushnil(E->L); // First key
    //        while (lua_next(E->L, headersIdx) != 0)
    //        {
    //            // Uses 'key' (at index -2) and 'value' (at index -1)
    //            if (lua_isstring(E->L, -2))
    //            {
    //                std::string key(lua_tostring(E->L, -2));
    //                std::string value(lua_tostring(E->L, -1));
    ...
    //    lua_pushvalue(E->L, callbackIdx);
    //    int funcRef = luaL_ref(E->L, LUA_REGISTRYINDEX);
    //    if (funcRef >= 0)
    //    {
    //        E->httpManager.PushRequest(new HttpWorkItem(funcRef, httpVerb, url, body, bodyContentType, headers));
    //    }
    //    else
    //    {
    //        luaL_argerror(E->L, callbackIdx, "unable to make a ref to function");
    //    }
    ...
    int ClearSpellEvents(Eluna* E)
    {
        ...
        else
        {
            uint32 entry = E->CHECKVAL<uint32>(1);
            uint32 event_type = E->CHECKVAL<uint32>(2);
            E->SpellEventBindings->Clear(Key((Hooks::SpellEvents)event_type, entry));
        }
        return 0;
    }
    ...
    int GetOwnerHalaa(Eluna* E)
    {
        ...
        E->Push(halaa->GetControllingFaction());
        E->Push(halaa->GetSlider());
     ...
     */
    int SetOwnerHalaa(Eluna* E)
    {
        uint16 teamId = E->CHECKVAL<uint16>(1);
        ...
        {
            return luaL_argerror(E->L, 1, "0 for Alliance or 1 for Horde expected");
        }
    ...
    int GetSpellInfo(Eluna* E)
    {
        ...
        E->Push(sSpellMgr->GetSpellInfo(spellId));
        return 1;
